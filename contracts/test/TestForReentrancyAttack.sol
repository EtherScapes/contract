...
contract TestForReentrancyAttack is IERC1155TokenReceiver {
    ...
    /*function attack(uint256 _totalToMint) external {
        ...
        TileFactory(factoryAddress).mint(1, address(this), 1, "");
        }*/

    // We attempt a reentrancy attack here by recursively calling the TileFactory
    // that created the MyCollectible ERC1155 token that we are receiving here.
    // We expect this to fail if the TileFactory.mint() function defends against
    // reentrancy.
