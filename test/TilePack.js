...
const TilePack = artifacts.require("../contracts/TilePack.sol");
...
const checkOption = async (
  mTilePack, index, maxQuantityPerOpen, hasGuaranteedClasses
) => {
  const option = await mTilePack.optionToSettings(index);
...
contract("TilePack", (accounts) => {
  ...
  before(async () => {
    ...
    mTilePack = await TilePack.new(
      proxy.address,
      myCollectible.address
    );
    await myCollectible.transferOwnership(mTilePack.address);
  });
  ...
  describe('#constructor()', () => {
    it('should set proxyRegistryAddress to the supplied value', async () => {
      assert.equal(await mTilePack.proxyRegistryAddress(), proxy.address);
      assert.equal(await mTilePack.nftAddress(), myCollectible.address);
    });
  });
